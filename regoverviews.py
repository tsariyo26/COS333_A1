...
import textwrap
import sys
def fetch_classes(dept=None, num=None, area=None,
title=None, db_path="reg.sqlite"):
    """
    Queries the SQLite database to retrieve 
    matching class details based on the provided filters.
    """
    query = """
    ...
    query += (" ORDER BY crosslistings.dept ASC, "
    "crosslistings.coursenum ASC, classes.classid ASC")
    try:
...
def print_table(results):
    ...
    for row in results:
        ...
        row = '%5s %4s %6s %4s %s' % (classid, dept,
        coursenum, area, title)
        wrapped_text = textwrap.wrap(row, width=72,
...
def main():
    ...
    results = fetch_classes(dept=args.d, num=args.n,
